...
decision of \texttt{TreeNose}. After that, we, the two authors, to % change the developers to the two authors to refer to the right people
...
that are detected by both \texttt{TreeNose} and us among the % change the developers to us to refer to the right people
samples selected from the other tools, the False Positive as the number of code
smells that are detected by \texttt{TreeNose} but not by us, and % change the developers to us to refer to the right people
...
% Remove "up to" 16 to make it precise
To answer the RQ2 and RQ3, we implemented \texttt{TreeNose} in 16
...
% """Another threat is related to the annotation study. To evaluate the performance
% of \texttt{TreeNose}, we conducted an annotation study with 2 developers.
% """
...
% Note: in my view, the third threat is not a threat to validity but a limitation, so we may delete it?
